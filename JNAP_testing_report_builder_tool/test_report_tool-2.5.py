...
class create_custom_dataframe():
    ...
    def group_transaction_table(self,group_by_selection,transaction_dataframe):
        
        if group_by_selection[0] == 0 :##State
            transaction_dataframe = transaction_dataframe[['GPS_State',
            'Qty_Sold',
            'Extended_Cost',
            'Extended_Sales']]
            transaction_dataframe = transaction_dataframe.groupby(['GPS_State']).agg(np.sum)
            transaction_dataframe['Profit'] = transaction_dataframe['Extended_Sales'] - transaction_dataframe['Extended_Cost']
            ...
            'Extended_Cost',
            'Extended_Sales']]
            transaction_dataframe = transaction_dataframe.groupby(['GPS_State',
            'GPS_City',
            ...
            'Item_ID',
            'Ent_Item_Number',
            'Item_Desc']).agg(np.sum)

            transaction_dataframe['Item_Overall_Cost'] = transaction_dataframe.groupby('Item_ID').Extended_Cost.transform(np.sum)
            transaction_dataframe['Item_Overall_Sales'] = transaction_dataframe.groupby('Item_ID').Extended_Sales.transform(np.sum)
            transaction_dataframe['Profit'] = transaction_dataframe['Extended_Sales'] - transaction_dataframe['Extended_Cost']
            transaction_dataframe['Item_Overall_Profit'] = transaction_dataframe.groupby('Item_ID').Profit.transform(np.sum)
            transaction_dataframe['Percent_Profit'] = (transaction_dataframe['Profit'] / transaction_dataframe['Item_Overall_Profit']) * 100
...
class write_to_excel():
    ...
    def __init__(self,final_report,cwd): 
        ...
        self.query_path = cwd + '/queries.csv'
        self.login = os.getlogin()
        self.datetime = datetime.datetime.now()
        self.datetime_str = self.datetime.strftime("%Y-%m-%d %H:%M")
        #self.writer = ExcelWriter(self.excel_path)
        self.exported_to_excel = self.export_to_excel(final_report, cwd)
        try:
            self.exported_to_csv = self.export_to_csv(self.query_path,self.datetime_str, self.login, final_report.transaction_dataframe.query)
        except AttributeError:
            pass
        try:
            self.exported_to_csv = self.export_to_csv(self.query_path,self.datetime_str, self.login, final_report.query)
        except AttributeError:
            pass

    def export_to_excel(self, final_report, cwd):
...
if __name__ =='__main__':
    ...
    print("******************************************")
    final_report = report()
    write_to_excel = write_to_excel(final_report.report,universal_settings.cwd)
